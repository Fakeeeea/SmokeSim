...
grid2d get_grid2d(const ivec2 size) {
    grid2d out;
    glm_ivec2_copy((ivec2){size[0],size[1]}, out.size);

    out.vx_idx = out.vy_idx = 0;
    out.smoke_idx = 0;
...
void gen_grid_textures2d(grid2d *grid, const float t_ambient) {
    grid->vx[0] = create_texture2((ivec2){grid->size[0]+1, grid->size[1]}, GL_R32F, GL_RED, GL_FLOAT, GL_LINEAR, NULL, GL_CLAMP_TO_BORDER, NULL);
    grid->vx[1] = create_texture2((ivec2){grid->size[0]+1, grid->size[1]}, GL_R32F, GL_RED, GL_FLOAT, GL_LINEAR, NULL, GL_CLAMP_TO_BORDER, NULL);

    grid->vy[0] = create_texture2((ivec2){grid->size[0], grid->size[1]+1}, GL_R32F, GL_RED, GL_FLOAT, GL_LINEAR, NULL, GL_CLAMP_TO_BORDER, NULL);
    grid->vy[1] = create_texture2((ivec2){grid->size[0], grid->size[1]+1}, GL_R32F, GL_RED, GL_FLOAT, GL_LINEAR, NULL, GL_CLAMP_TO_BORDER, NULL);
...
void gen_grid_pressure_solve_textures2d(grid2d* grid) {
    ivec2 dims = {grid->size[0], grid->size[1]};
    grid->pyramids_count = 0;
    while(dims[0] >= 4 && dims[1] >= 4) {
        dims[0] >>= 1;
        dims[1] >>= 1;
        grid->pyramids_count++;
    }
    ...
    grid->pressure_pyramid = malloc(grid->pyramids_count * sizeof(texture2));
    grid->solid_pyramid = malloc(grid->pyramids_count * sizeof(texture2));
    grid->residual_pyramid = malloc(grid->pyramids_count * sizeof(texture2));

    ivec2 current_s = {grid->size[0], grid->size[1]};
...
void swap_velocity_buffers2d(grid2d* grid) {
    swap_vx_buffer2d(grid);
    swap_vy_buffer2d(grid);
}
